...
mil = fread("../data/raw/mil_union_army.csv")
dis = fread('../data/raw/dis_union_army.csv')
msr = fread('../data/raw/msr_union_army.csv')
cen = fread("../data/raw/cen_union_army.csv")
...
Combine all rows with the same `recidnum`

```{r}

duckdb::duckdb_register(con, "data1_raw", full_data1)
duckdb::duckdb_register(con, "data2_raw", full_data2)


full_data1 <- dbGetQuery(con, "
  SELECT 
    CAST(recidnum AS VARCHAR) as recidnum,
    * EXCLUDE recidnum
  FROM data1_raw
  WHERE recidnum IS NOT NULL
  QUALIFY ROW_NUMBER() OVER (PARTITION BY recidnum ORDER BY recidnum) = 1
") |> as.data.table()

full_data2 <- dbGetQuery(con, "
  SELECT 
    CAST(recidnum AS VARCHAR) as recidnum,
    * EXCLUDE recidnum
  FROM data2_raw
...
# Re-register deduplicated versions
...
duckdb::duckdb_register(con, "full_data2", (full_data2 |> mutate(recidnum = as.character(recidnum))))


```


Isolate all rows where at least one column mentions 'TUBERCULOSIS','CONSUMPTION', or 'PHTHISIS'
...
(
  select *
  from full_data1
  full join full_data2
    on full_data1.recidnum = full_data2.recidnum
)
as tb

WHERE REGEXP_MATCHES(a_clm01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clm02,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clm03,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clm04,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clm05,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clm07,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clm08,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clst01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clst02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(a_clst07,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(afdesc01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(afdesc02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(afdesc07,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn03,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn04,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn05,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn07,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn08,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn09,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn10,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn11,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn12,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn13,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn14,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn16,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn19,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dschrn20,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs02,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs03,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs04,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs05,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs06,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs07,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs08,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs09,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs10,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs12,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs16,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(dthcs18,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(gen_rd_cause_6icd, 'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(g_rel01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(g_rel02,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(g_rel06,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(g_rel07,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_cmt2,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_eff101,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_eff102,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_eff103,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_eff104,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_eff107,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_eff201,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_fev1,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_fev2,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_rel101,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(i_rel102,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc03,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc04,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc05,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc06,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc07,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc08,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc09,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc10,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc11,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc12,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc13,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc14,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc15,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc16,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc17,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc18,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc19,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(ilwdsc20,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(l_rel01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(m_rel01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(n_rel01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(othrrn01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(othrrn02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_cmt1,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_cmt2,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_cmt3,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_cmt5,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(pc_ds1,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(pc_ds2,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(pc_ds3,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_lrs1,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_lrs2,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_lrs3,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_lrs4,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_lrs5,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_lrs7,    'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_rel01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(p_rel02,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(q_rel01,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(rd_cause,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(rd_ccaus,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result03,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result04,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result05,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result06,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result08,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(result11,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc1rmks,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc2rmks,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc4rmk01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc6rmk01,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc6rmk02,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc6rmk03,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc6rmk04,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc6rmk05,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(sc6rmk08,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(unftc1_1,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(unftc1_2,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(unftc1_3,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(unftc2_1,  'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(x_ext11,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(x_ext21,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS')
  OR REGEXP_MATCHES(x_ext41,   'TUBERCULOSIS|CONSUMPTION|PHTHISIS');


```

(Add a TB identifier label and connect to database)

```{r}

duckdb_register(con, "tb", tb |> mutate(recidnum = as.character(recidnum)))
#duckdb_register(con, "full_data1", full_data1 |> mutate(recidnum = as.character(recidnum)))
#duckdb_register(con, "full_data2", full_data2 |> mutate(recidnum = as.character(recidnum)))
...
# Standardize NAs, coerce numerics, isolate year in date
...
all_birth <- remove_blank(all_birth) |>
  mutate(
    a_age = as.numeric(a_age),
    across(rb_date1:rb_date2, ~as.numeric(substr(.x, 1, 4))),
    across(recbyr_0:a_xmdate, ~as.numeric(substr(.x, 1, 4))),
    across(lstag1_1:lstag2_3, as.numeric),
    across(lstdt1_1:lstdt2_3, ~as.numeric(substr(.x, 1, 4))),
    across(recage_5:recabd_1, as.numeric),
  ) 
...
all_birth <- all_birth |>
  mutate(
    ...
    # birth yr via 1900 census when directly recorded
    !is.na(recbyr_0) & recbyr_0 <2000 ~
      recbyr_0,
    
    # birth yr via age at census (in order of reliability)
    !is.na(recabd_0) & recabd_0 < 110 & recabd_0 > 35 ~
      1900 - recabd_0,
    !is.na(recabd_1) & recabd_1 < 110 & recabd_0 > 45 ~
      1910 - recabd_1,
    !is.na(recage_6) & recage_6 < 110 ~
      1860 - recage_6,
    !is.na(recage_5) & recage_5 < 110 ~
      1850 - recage_5,
    
    # birthyear from mandatory physical exam given when applying for pension
    !is.na(a_xmdate) ~ a_xmdate - a_age,
    
    # use TRUSTED enlistment byr's w/ quality code "B"(subject to age inflation)
    rb_dtqc1 == "B"  ~ rb_date1,
    rb_dtqc2 == "B"  ~ rb_date2,
    
    
    # use all enlistment byrs
    !is.na(rb_date1) ~ rb_date1,
    TRUE ~ rb_date2
)) |>
  mutate(
    x1_1 = lstdt1_1 - lstag1_1,
    x1_2 = lstdt1_2 - lstag1_2,
    x1_3 = lstdt1_3 - lstag1_3,
    x2_1 = lstdt2_1 - lstag2_1,
    x2_2 = lstdt2_2 - lstag2_2,
    x2_3 = lstdt2_3 - lstag2_3
  ) |>
  
  # Step 3: compute row-wise mean of all six differences
  mutate(
    lst_byr_mean = rowMeans(across(x1_1:x2_3), na.rm = TRUE),
    BYR = coalesce(BYR, lst_byr_mean),
    BYR = case_when(
      BYR < 1865 & BYR > 1780 ~BYR,
      TRUE ~ NA
    )
  )
...
all_death = remove_blank(all_death) |>
  mutate(across(everything(), as.character)) |>
  mutate(
    DYR = case_when(
      !is.na(rd_date) ~ substr(rd_date, 1,4),
      TRUE ~ NA)
  )


all$DYR <- as.numeric(all_death$DYR)
all$BYR <- as.numeric(all_birth$BYR)
all$dth_age <- all$DYR - all$BYR
...
all <- all |>
  mutate(
    #Convert to character and set explicit missing values to NA
    across(c(gen_birthplace_icpsr1, gen_birthplace_icpsr2), 
           ~na_if(as.character(.x), "9999999")),
    across(c(gen_rb_cnty_icpsr1, gen_rb_cnty_icpsr2), 
           ~na_if(as.character(.x), "9999")),
    
    # Ensure quality codes are characters for comparison
    rb_plqc1 = as.character(rb_plqc1),
    rb_plqc2 = as.character(rb_plqc2),
    
    # Pick the BEST raw 6/7-digit ICPSR code
    best_bpl_raw = case_when(
      # If record 1 is explicitly marked as "Best", use it
      rb_plqc1 == "B" ~ gen_birthplace_icpsr1,
      # If record 2 is explicitly marked as "Best", use it
      rb_plqc2 == "B" ~ gen_birthplace_icpsr2,
      # Otherwise, default to record 1 if it exists
      !is.na(gen_birthplace_icpsr1) ~ gen_birthplace_icpsr1,
      # Fallback to record 2 if record 1 is missing
      !is.na(gen_birthplace_icpsr2) ~ gen_birthplace_icpsr2,
      # If both are NA, return NA
      TRUE ~ NA_character_
    ),
    
    # Coarsen into the final matching variable (State/Country level)
    bpl_match_clean = case_when(
      is.na(best_bpl_raw) ~ "Missing",
      nchar(best_bpl_raw) >= 7 ~ substr(best_bpl_raw, 1, 3), # Foreign
      nchar(best_bpl_raw) == 6 ~ substr(best_bpl_raw, 1, 2), # Native US State
      TRUE ~ "Missing"
    ),
    
    bpl_match_clean = as.factor(bpl_match_clean)
  )


```

Whether or not the soldier was wounded in battle
...
setDT(all)
all[, recidnum := as.character(recidnum)]


date_cols <- c("lstdt1_1", "mildfdt1", "lstdt1_2", "mildfdt2", 
               "lstdt1_3", "mildfdt3", "lstdt2_1", "mildfdt4", 
               "lstdt2_2", "mildfdt5", "lstdt2_3", "mildfdt6")
clean_cols <- c(date_cols, "DYR", "BYR")


lst_dt <- all[, c("recidnum", clean_cols, "TB_yn"), with = FALSE]

# Vectorized String Cleaning & Blanks Removal
for (col in clean_cols) {
  set(lst_dt, j = col, value = {
    x <- lst_dt[[col]]
    x[x == "" | is.na(x)] <- NA_character_
    x[grep("--$", x)] <- sub("--$", "00", x[grep("--$", x)])
    x[grep("--", x)] <- NA_character_
    as.numeric(substr(x, 1, 4))
  })
}

# Wipe out row-wise duplicates 

mat <- as.matrix(lst_dt[, clean_cols, with = FALSE])
mat[t(apply(mat, 1, duplicated))] <- NA

# Reassign cleaned values 
lst_dt[, (clean_cols) := as.data.table(mat)]

# Find min
lst_dt[, first_lst := do.call(pmin, c(.SD, na.rm = TRUE)), .SDcols = date_cols]

# Calc survival metrics
lst_dt[, `:=`(
  surv_time = as.numeric(DYR) - first_lst,
  age_lst   = first_lst - as.numeric(BYR),
  event     = 1
)]

# Filter out invalid entries
final_metrics <- lst_dt[
  !is.na(first_lst) & surv_time >= 0, 
  .(recidnum, first_lst, surv_time, age_lst, event)
]

# Merge back
all <- merge(all, final_metrics, by = "recidnum", all.x = TRUE)
...
job_clean <- job |>
  ...
  pivot_longer(
    cols = starts_with("ro_") | starts_with("gen_ro_"),
    names_to = c(".value", "job_num"),
    names_pattern = "(?:gen_)?ro_(.*?)(\\d+)"
  )|>
  mutate(
    clean_date_num = as.numeric(str_replace_all(fdt, "-", "0")),
    year = as.numeric(substr(clean_date_num, 1, 4))
  ) |>
  filter(is.na(year) | year <= 1865) |>
  group_by(recidnum) |>
  arrange(desc(year), desc(job_num), .by_group = TRUE) |>
  slice(1) |>
  ungroup()

lst_job_join <- job_clean |>
  select(
    recidnum, 
    "job_name" = job, 
    fdt, 
    jcd, 
    "lstjobdt_cln" = clean_date_num
  ) |>
  mutate(
    lstjobdt_cln = if_else(lstjobdt_cln < 18000000, NA_real_, lstjobdt_cln)
  ) |>
  # Deduplicate: keep the most recent job per person
  group_by(recidnum) |>
  arrange(desc(lstjobdt_cln), .by_group = TRUE) |>
  slice(1) |>
  ungroup()

all <- all |>
  left_join(lst_job_join, by = "recidnum")
...
hgt <- all |>
  select(recidnum,
    rh_feet1, rh_feet2, rh_feet3, rh_feet4,
    rh_inch1, rh_inch2, rh_inch3, rh_inch4,
    rh_fiqc1, rh_fiqc2, rh_fiqc3, rh_fiqc4,
    rh_date1, rh_date2, rh_date3, rh_date4
  ) |>
  mutate(
    # Remove any erroneous entries
    across(rh_feet1:rh_feet4, ~ if_else(.x<4 | .x>7, NA_real_, .x)),
    across(rh_inch1:rh_inch4, ~ if_else(.x<0 | .x>12, NA_real_, .x))
  )

hgt <- remove_blank(hgt) 

# Create empty columns to fill using case_when
hgt <- hgt |>
  mutate(
    INCH = NA,
    FEET = NA
...
hgt <- hgt |>
  mutate(
    # Fill in INCH with correct inch if 'B'
    INCH = case_when(
      rh_fiqc1 == "B" ~ rh_inch1,
      rh_fiqc2 == "B" ~ rh_inch2,
      rh_fiqc3 == "B" ~ rh_inch3,
...
# Checking rows to see how to make filter


i <- hgt |>
  filter(
    if_any(rh_fiqc1:rh_fiqc4, ~ .x %in% c("5","B"))
  )



# Find first recorded height

first_hgt <- hgt |>
  # Pivot feet, inch, and date all at the same time
  pivot_longer(
    cols = matches("^rh_(feet|inch|date)\\d+$"),
    names_to = c(".value", "measure_num"),
    names_pattern = "rh_(feet|inch|date)(\\d+)"
  ) |>
  mutate(
    # Replace all "--" with "00"
    date_clean = str_replace_all(date, "--", "00")
  ) |>
  # Keep only rows with valid 8-digit dates AND a recorded height
  filter(
    str_detect(date_clean, "^\\d{8}$"),
    !is.na(feet)
  ) |>
  # Group by person, sort by date, and grab the earliest one
  group_by(recidnum) |>
  arrange(date_clean, .by_group = TRUE) |>
  slice(1) |> 
  ungroup() |>
  select(
    recidnum,
    FEET = feet,
    INCH = inch,
    earliest_hgt_date = date_clean
  ) |>
  mutate(
    LST1_HGT_IN = FEET*12 + INCH
  )

hist(first_hgt$LST1_HGT_IN)

# Join back to `all`
all <- all |> left_join(first_hgt, by = "recidnum")
...
```{r write, include=F, eval=F}

tb <- all[1:2144,]
ntb <- all[2145:39338,]

write_csv(tb, "../data/in-progress/tb_v9.csv")
write_csv(ntb, "../data/in-progress/ntb_v8.csv")
